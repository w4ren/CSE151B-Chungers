#!/usr/bin/env bash
# Added by Codex: 8k best-of-4 candidate pool for the fixed eval-40 slice.
...
OUT_DIR="${2:-codex_added/results/vllm_eval40_8k_bestof4_2gpu}"
SEED_RAW_DIR="${SEED_RAW_DIR:-codex_added/results/vllm_eval40_8k_finalized_2gpu/raw}"
LIMIT="${LIMIT:-40}"
OFFSET="${OFFSET:-0}"
GPUS="${GPUS:-0,1}"
...
REASON_VLLM_BATCH_SIZE="${REASON_VLLM_BATCH_SIZE:-4}"
...
echo "Reasoner: ${NUM_SAMPLES} samples, ${MAX_NEW_TOKENS} tokens, max_model_len=${REASON_MAX_MODEL_LEN}, chunk=${REASON_VLLM_BATCH_SIZE}"
echo "Finalizer: LoRA, max_input_tokens=${FINAL_MAX_INPUT_TOKENS}, max_new_tokens=${FINAL_MAX_NEW_TOKENS}"
...
for shard_idx in "${!GPU_IDS[@]}"; do
    ...
    seed_path="${SEED_RAW_DIR}/shard_${shard_name}.jsonl"
    log_path="${OUT_DIR}/logs/reason_8k_bestof4_shard_${shard_name}.log"
    RAW_SHARDS+=("${raw_path}")

    if [[ ! -s "${raw_path}" && -s "${seed_path}" ]]; then
        cp "${seed_path}" "${raw_path}"
        echo "Seeded shard ${shard_name} with existing sample-0 raw traces from ${seed_path}"
    fi
...
wait_for_stage "8k best-of-4 reasoner" "${PIDS[@]}"
PIDS=()
...
"${PYTHON_BIN}" codex_added/scripts/25_analyze_candidate_pool.py \
    ...
    --offset "${OFFSET}" \
    --limit "${LIMIT}" \
    --responses "${RAW_SHARDS[@]}" \
    --labels raw_shard_00 raw_shard_01 \
    --output "${OUT_DIR}/raw_candidate_diagnostics.jsonl" \
    --summary-output "${OUT_DIR}/raw_candidate_summary.json"

for shard_idx in "${!GPU_IDS[@]}"; do
    shard_name="$(printf "%02d" "${shard_idx}")"
    raw_path="${OUT_DIR}/raw/shard_${shard_name}.jsonl"
    final_path="${OUT_DIR}/finalized/shard_${shard_name}.jsonl"
    log_path="${OUT_DIR}/logs/finalize_lora_bestof4_shard_${shard_name}.log"
...
"${PYTHON_BIN}" codex_added/scripts/25_analyze_candidate_pool.py \
    ...
    --offset "${OFFSET}" \
    --limit "${LIMIT}" \
    --responses "${FINAL_SHARDS[@]}" \
    --labels final_shard_00 final_shard_01 \
    --output "${OUT_DIR}/final_candidate_diagnostics.jsonl" \
    --summary-output "${OUT_DIR}/final_candidate_summary.json"
...
"${PYTHON_BIN}" codex_added/scripts/26_select_budget_candidates.py \
    ...
    --responses "${FINAL_SHARDS[@]}" \
    --labels final_shard_00 final_shard_01 \
    --output "${OUT_DIR}/selected_8k_bestof4_consensus.jsonl" \
    --score

echo "Done. Main scored output: ${OUT_DIR}/selected_8k_bestof4_consensus.jsonl"
